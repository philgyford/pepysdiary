...
class TestCommentForSpamTestCase(TestCase):
    ...
    def post_comment(self, data=None):
        ...
        return self.client.post("/annotations/post/", data, REMOTE_ADDR="1.2.3.4")
    ...
    def test_calls_akimset_api(self):
        "Posting an annotation when there's an API key should call Akismet"
    ...
    def test_does_not_publish_spam_comment(self):
        ...
        annotation = Annotation.objects.first()
        self.assertFalse(annotation.is_public)
    ...
    def test_spam_comment_generates_message_with_managers(self):
        ...
        self.assertEqual(
            ...
            '<a href="mailto:terry@example.org?subject=Flagged annotation (ID: '
            f'{annotation.id})">email me</a> '
    ...
    def test_standard_user_is_tested(self):
        ...
        self.log_user_in(is_staff=False, is_superuser=False, is_trusted_commenter=False)
        self.add_response("false")
        self.post_comment()
    ...
    def test_staff_user_is_not_tested(self):
        ...
        self.log_user_in(is_staff=True, is_superuser=False, is_trusted_commenter=False)
        self.add_response("false")
        self.post_comment()
    ...
    @responses.activate
    def test_trusted_commenter_is_not_tested(self):
        "Trusted commenters should not have their comments tested by Akismet"
        self.log_user_in(is_staff=False, is_superuser=False, is_trusted_commenter=True)
        self.add_response("false")
        self.post_comment()
    ...
    @responses.activate
    def test_logged_in_user_data_with_url(self):
        "If the user is logged in, and has a URL, it should be sent to Akismet"
        self.log_user_in(url="https://example.org/terry")
        self.add_response("false")
        self.post_comment(data={"comment": "Hello"})

        params = self.get_request_params(responses.calls[0].request)
        self.assertEqual(params["comment_author_url"], "https://example.org/terry")
