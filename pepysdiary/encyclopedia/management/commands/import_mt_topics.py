...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option(
            ...
            default=True,
            help=(
                "Import all Encyclopedia Topics from legacy MT MySQL database. "
                "(Default)",
            ),
        ),
    )
    ...
    def handle(self, *args, **options):
        ...
        cur.execute(
            "SELECT entry_id, entry_category_id, entry_title, "
        ...
        for row in rows:
            ...
            for meta_row in meta_rows:
                if (
                    meta_row["entry_meta_type"] == "field.latitude"
                    and meta_row["entry_meta_vchar_idx"] != ""
                ):
                    topic.latitude = meta_row["entry_meta_vchar_idx"]

                if (
                    meta_row["entry_meta_type"] == "field.longitude"
                    and meta_row["entry_meta_vchar_idx"] != ""
                ):
                    topic.longitude = meta_row["entry_meta_vchar_idx"]

                if (
                    meta_row["entry_meta_type"] == "field.map_category"
                    and meta_row["entry_meta_vchar_idx"] != ""
                ):
                    if meta_row["entry_meta_vchar_idx"] in valid_map_categories:
                        ...
                        print(
                            "INVALID MAP CATEGORY: '%s' for Entry ID '%s'"
                            % (meta_row["entry_meta_vchar_idx"], row["entry_id"])
                        )

                if (
                    meta_row["entry_meta_type"] == "field.wikipedia_title"
                    and meta_row["entry_meta_vchar_idx"] != ""
                ):
                    topic.wikipedia_fragment = meta_row["entry_meta_vchar_idx"]

                if (
                    meta_row["entry_meta_type"] == "field.zoom"
                    and meta_row["entry_meta_vchar_idx"] != ""
                ):
                    topic.zoom = meta_row["entry_meta_vchar_idx"]

                if (
                    meta_row["entry_meta_type"] == "field.shape"
                    and meta_row["entry_meta_vclob"] != ""
                ):
            ...
            for meta_row in meta_rows:
                if (
                    meta_row["entry_meta_type"] == "field.thumbnail_image"
                    and meta_row["entry_meta_vinteger_idx"] == 1
                ):
                    topic.thumbnail = "encyclopedia/thumbnails/%s.jpg" % (topic.pk)
                    topic.save()

            # 6) FETCH AND ADD CATEGORIES.

            cur.execute(
                "SELECT placement_category_id FROM mt_placement "
                "WHERE placement_entry_id='%s'" % (row["entry_id"])
            )
            cat_rows = cur.fetchall()
            for cat_row in cat_rows:
                try:
                    category = Category.objects.get(pk=cat_row["placement_category_id"])
                    topic.categories.add(category)
                except Category.DoesNotExist:
                    print(
                        "INVALID CATEGORY ID: '%s' for Entry ID '%s'"
