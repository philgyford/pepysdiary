...
def is_akismet_spam(sender, comment, request):
    ...
    if comment.user and (comment.user.is_staff or comment.user.is_trusted_commenter):
    ...
    r = requests.post(
        "https://" + settings.PEPYS_AKISMET_API_KEY + "." + AKISMET_CHECK_URL,
        data=parameters,
        headers=headers,
    )
...
def test_comment_for_spam(sender, comment, request, **kwargs):
    ...
    so won't work with Annotation. So a load of the stuff below is
    ...
    if is_akismet_spam(sender, comment, request):
        ...
        if len(managers) > 0:
            message_content += (
                " If it wasn't then "
                f'<a href="mailto:{managers[0][1]}?subject=Flagged annotation '
                f'(ID: {comment.id})">email me</a> to have it published.'
            )
