...
class Command(BaseCommand):
    ...
    Usage:
    $ ./manage.py import_mt_posts

    And afterwards, do this:
    $ ./manage.py sqlsequencereset 'news'
    ...
    def handle(self, *args, **options):
        ...
        cur.execute(
            ...
            "FROM mt_entry WHERE entry_blog_id='%s'" % (settings.MT_NEWS_BLOG_ID)
        )
        ...
        for row in rows:
            ...
            post = Post(
                ...
                intro=intro,
                text=text,
                date_published=row["entry_authored_on"].replace(tzinfo=pytz.utc),
